...
class self_DataLoader(Dataset):
    def __init__(self, root, train=True, dataset='MSTAR', seed=1, nway=5):
        ...
        self.transform_SAR = tv.transforms.Compose([
            tv.transforms.Grayscale(1),  # single channel
    ...
    def load_batch_data(self, train=True, batch_size=16, nway=5, num_shots=1):
        if train:
            ...
            x = []
            label_y = []       # task label (0..nway) where nway = unseen
            one_hot_y = []
            class_y = []       # real label in original dataset
            ...
            for i in range(batch_size):
                ...
                # ✅ التعديل الأساسي: إدخال unseen فعليًا في الاختبار بنسبة 50%
                if random.random() < 0.5:
                    positive_class = nway   # unseen query
                else:
                    positive_class = random.randint(0, nway - 1)
                ...
                if positive_class == nway:
                    ...
                    # كما هو في الكود الأصلي: قيمة رمزية للفئة الحقيقية لعينات unseen
                    ...
                    label2class[nway] = test_sampled_classes[0]
